...
class SqsSourceOptions(parameters: CaseInsensitiveMap[String]) extends Logging {
  ...
  /** Maximum age of a file that can be found in this directory, before it is
    * ignored. For the first batch all files will be considered valid.
    *
    * The max age is specified with respect to the timestamp of the latest file,
    * and not the timestamp of the current system. That this means if the last
    * file has timestamp 1000, and the current system time is 2000, and max age
    * is 200, the system will purge files older than 800 (rather than 1800) from
    * the internal state.
    *
    * Default to a week.
    */
  ...
  val fetchIntervalMilliSeconds: Int = parameters
    .get("sqsFetchIntervalMilliSeconds")
    .map { str =>
      Try(str.toInt).toOption.filter(_ > 0).getOrElse {
        throw new IllegalArgumentException(
  ...
  val longPollWaitTimeSeconds: Int = parameters
    .get("sqsLongPollingWaitTimeSeconds")
    .map { str =>
  ...
  val maxConnections: Int = parameters
    .get("sqsMaxConnections")
    .map { str =>
      Try(str.toInt).toOption.filter(_ > 0).getOrElse {
        throw new IllegalArgumentException(
  ...
  val endpoint: Option[String] = parameters.get("endpoint")
  ...
  val ignoreFileDeletion: Boolean =
    withBooleanParameter("ignoreFileDeletion", false)

  /** Whether to check new files based on only the filename instead of on the
    * full path.
    *
    * With this set to `true`, the following files would be considered as the
    * same file, because their filenames, "dataset.txt", are the same:
    *   - "file:///dataset.txt"
    *   - "s3://a/dataset.txt"
    *   - "s3n://a/b/dataset.txt"
  ...
  val useInstanceProfileCredentials: Boolean =
    withBooleanParameter("useInstanceProfileCredentials", false)
