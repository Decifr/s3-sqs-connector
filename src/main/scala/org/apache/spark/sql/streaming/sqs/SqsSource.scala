...
class SqsSource(
  ...
  metadataLog.allFiles().foreach { entry =>
    sqsClient.sqsFileCache
      .add(entry.path, MessageDescription(entry.timestamp, true, ""))
  }
  sqsClient.sqsFileCache.purge()

  logInfo(
    s"maxFilesPerBatch = $maxFilesPerTrigger, maxFileAgeMs = $maxFileAgeMs"
  )
  ...
  private def fetchMaxOffset(): FileStreamSourceOffset = synchronized {
    ...
    /** All the new files found - ignore aged files and files that we have seen.
      ...
      */
    val batchFiles = sqsClient.sqsFileCache.getUncommittedFiles(
      maxFilesPerTrigger,
      shouldSortFiles
    )
    ...
    if (batchFiles.nonEmpty) {
      ...
      metadataLog.add(
        ...
        batchFiles.map { case (path, timestamp, receiptHandle) =>
          FileEntry(
            path = path,
            timestamp = timestamp,
